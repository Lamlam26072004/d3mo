...
const Mail = {
  sendResetPassword: async (email, token) => {
    try {
      const info = await transporter.sendMail({
        from: '"Tempus Chroniker xin thông báo !" <admin@ethereal.email>',
        to: email,
        ...
        html: `
          <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
            ...
            <p>Xin chào,</p>
            <p>Bạn đã yêu cầu đặt lại mật khẩu cho tài khoản của mình tại <strong>Tempus Chroniker</strong>. Vui lòng nhấn vào liên kết dưới đây để thay đổi mật khẩu của bạn:</p>
            ...
            <p style="margin-top: 30px; color: #888;">Trân trọng,<br/>Đội ngũ hỗ trợ Tempus Chroniker</p>
          </div>
        `,
      });
  ...
  sendOrderConfirmation: async (email, order) => {
    const info = await transporter.sendMail({
      from: '"Tempus Chroniker" <admin@ethereal.email>',
      to: email,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #ddd; padding: 20px; border-radius: 8px;">
          ...
          <p style="text-align: center; margin-top: 30px; font-size: 14px; color: #555;">Tempus Chroniker xin cảm ơn quý khách!</p>
        </div>
      `,
    });
    ...
    const info = await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #ddd; padding: 20px; border-radius: 8px;">
          ...
          <p style="text-align: center; margin-top: 30px; font-size: 14px; color: #555;"> xin cảm ơn quý khách!</p>
        </div>
      `,
    });
